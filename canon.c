  ...
  if (!stms) 
    return next(prevstms, 
		T_StmList(T_Jump(T_Name(done), Temp_LabelList(done, NULL)), 
			  NULL), done);
  if (stms->head->kind == T_JUMP || stms->head->kind == T_CJUMP) { //a block starts with JUMP, move this JUMP to previous block.
    ...
    return next(prevstms, T_StmList(T_Jump(T_Name(lab), Temp_LabelList(lab, NULL)), 
			     stms), done); //add a JUMP at the end of previous block, so that it jumps to current block.
  }
  else { //this should be directly merged to the previous block.
    prevstms->tail = stms;
...
static void trace(T_stmList list)
{
  ...
  S_enter(block_env, lab->u.LABEL, NULL); //corresponding to ln 281
  if (s->kind == T_JUMP) {
  ...
  /* we want false label to follow CJUMP */
    ...
    else { //true and false are both NULL if they are processed before.
      Temp_label false = Temp_newlabel();
      last->tail->head = T_Cjump(s->u.CJUMP.op, s->u.CJUMP.left,
				 s->u.CJUMP.right, s->u.CJUMP.true, false);
      last->tail->tail = T_StmList(T_Label(false), getNext()); //lack a JUMP instruction??
    }
  }
  else assert(0);
}
...
static T_stmList getNext()
{
  if (!global_block.stmLists)
    ...
    else { //tricky here. If a block is ended, its binding will be NULL in th env!
      global_block.stmLists = global_block.stmLists->tail; //Note: global_block changed, original block not changed!
      return getNext();
    }
  }
}
