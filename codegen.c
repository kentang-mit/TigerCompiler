...
static Temp_tempList munchArgs(int i, T_expList args){
	...
	emit(AS_Oper(String("movq `s0 `d0"), L(dest_reg, NULL), L(arg_reg,NULL), NULL));
	...
	switch(e->kind){
		...
		case T_BINOP:{
			...
			switch(e->u.BINOP.op){
				...
				case T_mul:{oper=String("mulq"); break;} 
				default: oper = String("");
			}
			Temp_temp result = Temp_newtemp();
			...
			if(result != left_op){
				if(e1->kind != T_CONST) emit(AS_Oper("movq `s0, `d0", L(result, NULL), L(left_op, NULL), NULL));
				else{
			...
			if(e->u.BINOP.op != T_div){
				emit(AS_Oper(String(buffer), L(result, NULL), L(result, L(right_op, NULL)), NULL));
			}
			else{
				//special division for x86-64
				Temp_temp rax = F_RV();
				Temp_temp rdx = F_RDX();
				emit(AS_Oper(String("movq `s0 `d0"), L(rax, NULL), L(left_op, NULL), NULL));
				emit(AS_Oper(String("cltq"), NULL, NULL, NULL));
				emit(AS_Oper(String("idivq `s0"), L(rax, NULL), L(right_op, L(rax, L(rdx, NULL))), NULL));
				emit(AS_Oper(String("movq `s0 `d0"), L(result, NULL), L(rax, NULL), NULL));
			}
			return result;
		}
